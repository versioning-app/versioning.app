...
export function BillingManagement({ prices }: { prices: Stripe.Price[] }) {
  const [interval, setInterval] =
    useState<Stripe.Price.Recurring.Interval>('year');
  const products = prices.map((price) => price.product as Stripe.Product);

  const getProductPrices = (product: Stripe.Product) =>
    prices.filter(
      (price) =>
        price.product === product && price.recurring?.interval === interval
    )?.[0];
      ...
      <li>
        {products.map((product) => (
          <BillingProduct
            key={product.id}
            product={product}
            price={getProductPrices(product)}
            period={interval}
          />
        ))}
      </li>
...
export function BillingProduct({
  product,
  price,
  period,
}: {
    ...
    <div>
      ...
      {price && (
        <div>
          ...
          <Button
            onClick={async () => createCheckoutSession({ priceId: price.id })}
          >
            Subscribe
          </Button>
        </div>
